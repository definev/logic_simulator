...
class BinaryIList<T> {
  ...
  BinaryIList<T> lockAt(T element, T Function(T element) onUpdate) {
    ...
    final unlockList = _list;
    unlockList[index] = updatedElement;
  ...
  BinaryIList<T> unlockAt(T element, T Function(T element) onUpdate) {
    final index = listBinarySearch(_list, element, comparator: _compare);
    if (index == -1) return this;
    _selectedSet.remove(element);
    final updatedElement = onUpdate(element);
...
extension IListToBinaryListX<T> on List<T> {
  BinaryIList<T> binaryList(Comparator<T> compare) => BinaryIList(list: this, compare: compare);
}

extension BinaryIListX<T> on BinaryIList<T> {
  BinaryIList<T> binaryIList(Comparator<T> compare) => BinaryIList(
        list: list,
        compare: compare,
